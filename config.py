class BaseConfig:
    SIMULATION_STEPS = 50000  # DQNの学習には多くのステップが必要
...
class ConfigA(BaseConfig):
    ...
    """
    NAME = "Config A (標準)"
    BANDWIDTH_CENTER = 100       # 帯域幅の中心値
    BANDWIDTH_AMPLITUDE = 50     # 中心値からの振れ幅 (50~150で変動)
    BANDWIDTH_PERIOD = 500       # 変動の1周期の長さ (ステップ数)
...
class DqnTrainConfig(BaseConfig):
    ...
    BANDWIDTH_AMPLITUDE = 50     # 中心値からの振れ幅 (50~150で変動)
    BANDWIDTH_PERIOD = 500       # 変動の1周期の長さ (ステップ数)
    MAX_PACKETS_PER_STEP = 5
    BUFFER_PACKET_LIMIT = 100    # バッファの最大パケット数を固定
    BUFFER_BYTE_LIMIT = 500      # こちらは参考値とする
